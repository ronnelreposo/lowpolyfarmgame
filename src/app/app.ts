...
export class App {
  ...
  constructor() {
    ...
    console.log("1", planResult[0].map(t => t.taskId)); // ["loadPage", "loadResource", "loadImage"]

    const plantResult2 = planInternal(htn, new Set(["pageLoaded"])); // At  least the page is loaded, plan next step.
    console.log("2", plantResult2[0].map(t => t.taskId)); // ["loadResource", "loadImage"]
  }
}
